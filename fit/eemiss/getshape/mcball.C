...
const char* filename="sigh02.root";
const int NofSignal=50000;

const int input_tree_name = 0;
const double left_range = 0.07;
const double right_range = 0.03;
const double global_lef = 0.3;
const double global_right = 2.1;
...
void mcball()
{
	...
	else if(input_tree_name == 0 )
	{
		...
		TFile *f=new TFile("./sigh02.root","READ");
		TIter nextkey(f->GetListOfKeys());
		TKey *key;
		while (key = (TKey*)nextkey()){
			TTree *tree = (TTree *)key->ReadObj();
			TString gettreename(tree->GetName());
			TString getu(gettreename(1));
			getu+=".";
			getu+=gettreename(2);
			TString geth(gettreename(5));
			geth+=".";
			geth+=gettreename(6);
			double getmu=atof(getu);
			double getmh=atof(geth);
			//cout<<gettreename<<" "<<getmu<<" "<<getmh<<endl;
		
			low = getmu-left_range;
			high = getmu +right_range;

			reset=cball(gettreename,low,high,reset_1);
			...
			eff=nfit*1.0/NofSignal;
			tout->Fill();
			outfile<<getmu<<" "<<cbsigma<<" "<<n1<<" "<<alpha<<" "<<eff<<endl;
		}
